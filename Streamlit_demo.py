...
st.write(f"Out of the {df.shape[1]} columns, you might want to view only a subset. streamlit has a [multiset](https://streamlit.io/docs/api.html#streamlit.multiselect) widget for this.")
...
st.write("You can also static tables. As opposed to a data frame, with a static table you cannot sort by clicking a column header.")
st.table(df.groupby("room_type").price.mean().reset_index()\
    .sort_values("price", ascending = False).round(2)
    .assign(avg_price= lambda x: x.pop("price").apply(lambda y: "%.2f" % y))
    )

st.header("what is the distrubution of property price?")
st.write(
    """Select a custom price range from the side bar to update the histogram below displayed as a Plotly chart using [`st.plotly_chart`](https://streamlit.io/docs/api.html#streamlit.plotly_chart).""")
values = st.slider("Price range", float(df.price.min()), float(df.price.clip(upper=1000.).max()), (50., 300.))
f = px.histogram(df.query(f"price.between{values}"), x='price', nbins = 15, title = "Price distrubution")
